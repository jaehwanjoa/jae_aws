...
해당 환경은 Identity Center를 활용하는 SSO 환경으로써 액세스 키 인증 방식이 아닌 Role 기반 인증방식을 채택하였고 코드상의 보안성을 확대시켰습니다. 
...
```bash
	...
	modules
	└─ waf
	   ├─ main.tf #메인 실행 파일(실행 시 WAF가 대상 계정에 자동 배포됩니다.) 
   └─ variables.tf
```
## 3. 실행 방법
...
```
이후 전체적인 흐름은 아래와 같습니다.
```bash
SSO 로그인: aws sso login --profile ons-waf
Terraform 시작: terraform init
Terraform 배포 적용: terraform apply (CloudFront/Regional 
