...
def build_wf_summary(
    ...
    wf_info = (
        wildfire_report.get("wildfire") or {}
    )

    file_info = (
        wf_info.get("file_info") or {}
    )

    task_info = wf_info.get("task_info")

    if not task_info:
        wf_status = "BEGIN"
        wf_report_status = "아직 생성되지 않음"

        print("WildFire 분석 상태: BEGIN")
        print("WildFire Report: 아직 생성되지 않음")

    else:
        wf_status = "COMPLETED"
        wf_report_status = "생성됨"
                ...
                if not wf_hash:
                    ...
                    try:
                        ...
                        try:
                            ...
                            print(
                                f"MAIL_SENT_TO={len(MAIL_RECEIVERS)}"
                            )
